...
//   User, // For task owner icon
//   Calendar, // For due date icon
//   ClipboardList, // For status icon
//   AlertTriangle // For priority icon
// } from 'lucide-react';

// // Import your custom useAuth hook
// // Path is relative from src/components/EmployeeDashboard/Taskemployee.jsx to src/context/AuthContext.jsx
...
// // --- CreateTaskModal Component (Integrated with useAuth for owner logic) ---
...
// }) => {
//   // Determine if task owner field should be disabled
//   const isTaskOwnerDisabled = currentUser?.isLoggedIn && currentUser?.role === 'employee';
...
//                       className={`w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white appearance-none pr-10 ${isTaskOwnerDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
//                       disabled={isTaskOwnerDisabled} // Disable if a regular user is logged in
//                     >
//                       {isTaskOwnerDisabled ? (
...
//     taskOwner: currentUser?.isLoggedIn && currentUser?.role === 'employee' ? currentUser.name : 'None', // Pre-fill with current user if employee
//     subject: '',
//     dueDate: '',
//     contact: '',
//     account: '',
...
//       });
//     } else {
//       // Reset form data for a new task (when opening Create form), pre-filling owner if employee
//       setFormData({
//         taskOwner: currentUser?.isLoggedIn && currentUser?.role === 'employee' ? currentUser.name : 'None',
//         subject: '',
//         dueDate: '',
//         contact: '',
//         account: '',
...
//       });
//     }
//   }, [editingTask, currentUser]); // Dependency on currentUser to reset owner on login/logout
...
//   // Helper to determine the actual task owner for API payload
//   const getActualTaskOwner = () => {
...
//     return formData.taskOwner === 'None' ? 'Amit Seth' : formData.taskOwner; // Default to Amit Seth if "None" selected by admin
//   };
...
//       alert('Subject and Due Date are required to create a task.');
//       return;
//     }
...
//       const newTaskData = {
//         ...formData,
//         taskOwner: getActualTaskOwner(), // Use the helper to set the owner
//       };
...
//     if (!formData.subject || !formData.dueDate) {
//       alert('Subject and Due Date are required to save and create a new task.');
//       return;
//     }
...
//       console.log('Attempting to save and create new task with data:', newTaskData);
//       const response = await fetch(`${API_BASE_URL}/task`, {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
...
//       alert('Subject, Due Date, and a task to edit are required for update.');
//       return;
//     }
...
//         ...formData,
//         taskOwner: getActualTaskOwner(), // Ensure owner is correctly set for update payload
//       };
//       console.log(`Sending update data for task ${editingTask._id}:`, updatedTaskData);
...
//   // --- Delete Task Handler ---
//   const handleDeleteTask = async (taskId, taskOwner) => {
//     // Permission check for delete
//     const canDelete = currentUser?.isLoggedIn &&
//                       (currentUser?.role === 'admin' || (currentUser?.role === 'employee' && currentUser.name === taskOwner));
...
//       // If still loading auth or no user, do not fetch tasks yet.
//       return;
//     }

//     try {
//       setLoading(true);
//       console.log('Attempting to fetch all tasks from:', `${API_BASE_URL}/task`);
//       const response = await fetch(`${API_BASE_URL}/task`);
...
//         // Important: apply user filter to fallback data as well
//         const fallbackData = (currentUser?.isLoggedIn && currentUser?.role === 'employee')
//           ? initialSampleTasks.filter(task => task.taskOwner === currentUser.name)
//           : initialSampleTasks;
...
//       if (currentUser.isLoggedIn && currentUser.role === 'employee') {
//         const filteredData = data.filter(task => task.taskOwner === currentUser.name);
//         setTasks(filteredData);
//         console.log(`Filtered tasks for employee ${currentUser.name}:`, filteredData);
//       } else {
//         // If admin, or not logged in, show all tasks
//         setTasks(data);
//         console.log('Displaying all tasks (Admin view or no user logged in).');
//       }
//     } catch (error) {
//       console.error('Error fetching tasks from backend:', error);
//       // Fallback to initial sample data on fetch error, with user filter
//       const fallbackData = (currentUser?.isLoggedIn && currentUser?.role === 'employee')
//         ? initialSampleTasks.filter(task => task.taskOwner === currentUser.name)
//         : initialSampleTasks;
//       setTasks(fallbackData);
//       console.log('Falling back to sample tasks due to fetch error or empty data, with user filter applied.');
//     } finally {
//       setLoading(false);
//     }
//   }, [currentUser, authLoading]); // Re-fetch when currentUser or authLoading changes

//   // Fetch tasks on component mount or when currentUser/authLoading changes
//   useEffect(() => {
//     fetchTasks();
//   }, [fetchTasks]);
...
//                             }}
//                             // Disable edit button if not admin AND not the owner of the task
//                             disabled={!(currentUser?.isLoggedIn && (currentUser?.role === 'admin' || (currentUser?.role === 'employee' && currentUser.name === task.taskOwner)))}
...
//                           </button> */}
//                           {/* <button
//                             onClick={() => handleDeleteTask(task._id, task.taskOwner)} // Pass owner for permission check
//                             // Disable delete button if not admin AND not the owner of the task
//                             disabled={!(currentUser?.isLoggedIn && (currentUser?.role === 'admin' || (currentUser?.role === 'employee' && currentUser.name === task.taskOwner)))}
...
const Taskemployee = () => {
  ...
  const getActualTaskOwner = () => {
    ...
    return formData.taskOwner === 'None' ? 'Amit Seth' : formData.taskOwner;
  };
                    ...
                    <tr key={task._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm whitespace-nowrap">
                        {/* <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => handleDeleteTask(task._id, task.taskOwner)}
                            disabled={!canEditOrDelete(task.taskOwner)}
                            className="p-1 rounded-full text-red-600 hover:bg-red-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Delete Task"
                          >
